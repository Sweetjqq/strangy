    ...
    <div class="ListData">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="策略信息" name="first">
          <CreateStrategyInfo ref="whateverName"></CreateStrategyInfo>
        </el-tab-pane>
        <el-tab-pane label="策略建议" name="second">
              ...
              <div class="only_echart">
                <div class="echart_left">
                  ...
                  <div style="width: 47%;display: inline-block;height: 100%">
                    <PieChart id="Debt1" :options='DebtData' ref="c7" :text='text1'></PieChart>
                  </div>
                </div>
                <div class="text_msg">
          ...
          <div class="saveBtn">
            <el-row>
              ...
              <el-button type="danger" @click="backList">返回</el-button>
            </el-row>
          </div>
        </el-tab-pane>
      </el-tabs>
...
<script>
  ...
  import CreateStrategyInfo from './CreateStrategyInfo.vue'
  ...
  export default {
    ...
    methods: {
      ...
      //创建策略
      CreateStrategy() {
        console.log(this.StrategyData, "this.StrategyData");
        let newJson = this.StrategyData;
        for (let i = 0; i < newJson.length; i++) {
          delete newJson[i].DueTime;
          delete newJson[i].Cash;
        ...
        for (let value of newJson) {
          ...
          FaceValueArray.push(value.FaceValue);
        }
        if (FaceValueArray.indexOf(0) > -1) {
          this.$message({
            message: "债券中的面值不能为0",
            type: 'warning'
          });
          ...
          if (newJson.length == 0) {
            ...
            axios({
              method: 'post',
              url: '/v1/StrategyApi/CreateStrategy',
              headers: {
                'Authorization': "bearer " + getCookie("Token"),
              },
              data: {
              ...
              if (response.data.ResultCode == 200) {
                console.log(response.data, "创建策略");
                this.$message({
                  message: "数据已提交，请到我的策略查看",
                  type: 'success'
                });
                setTimeout(() => {
      ...
      ShowMsgFromChild(data) {
        ...
        if (data.FaceValue > 0) {
          ...
          if (this.states == "edit") {
            for (let value of this.StrategyData) {
              if (value.Code == data.Code && data.FaceValue > 0) {
      ...
      handleClick(tab, event) {
        ...
        console.log(this.$refs.whateverName.queryStart(), "this.$refs.whateverName.queryStart()");
